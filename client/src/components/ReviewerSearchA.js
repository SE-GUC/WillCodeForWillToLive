...
class ReviewerSearchA extends Component {
    ...
    filterByPickedValue(){
      ...
      for (var i = 0; i < this.state.cases.length; i++) {
        if(this.state.pickedValue ==='reviewer'){
          if(this.state.text === this.state.cases[i].reviewer){
    ...
    render() {
      ...
      return(
        ...
        <form action="">
          ...
          <input type="radio" name="name" value ="reviewer" onChange={this.handleOptionChange} defaultChecked />اسم المراجع,
          <input type="radio" name="name" value="status" onChange={this.handleOptionChange}/> وضع حالة,
...
export default ReviewerSearchA;
