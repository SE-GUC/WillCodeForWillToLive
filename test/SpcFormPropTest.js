...
const Model = require('../models/SpcFormProperties')
...
test('Test changing the minimum spc capital', async () => {
  return fetch ('http://localhost:3000/api/sscFormProps', {
    method: 'PUT',
    body: JSON.stringify({minimumCapital: 100000}),
  ...
  .then(async _ => {
    ...
    expect(dataFromDb.length).toBe(1)
    expect(minimumCapital).toBe(100000)
  })
})